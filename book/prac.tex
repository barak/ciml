...
the addition of noisy or \concept{irrelevant features}.  Intuitively, an
...
~\\
  ...
  Moreover, the variance is proportional to the squared error of that ``predictor.''
\end{mathreview}
...
\TODOFigure{prac:spam}{show a bunch of emails spam/nospam sorted by model prediction, not perfect}
...
total of $1$M training examples.  The classification for a pixel in
...
$1000$ or $1$M data points) to measure the strength of this
...
For instance, the \feat{CheatingIsFun} strategy is designed explicitly to ensure that the approximation error is zero, and therefore isolating all error into the estimation error.
