...
And if this output sequence is incorrect, we will adjust the weights \emph{toward} the correct output sequence $\vy$ and away from the incorrect output sequence $\hat\vy$.
...
\end{align}
%
What this means is that we can build a graph like that in Figure~\ref{fig:srl:trellis}, with one vertical slice per time step ($l \in 1 \dots L$).\footnote{A graph of this sort is called a \concept{trellis}, and sometimes a \concept{lattice} in the literature.}
...
In particular, one suboptimal thing about the structured perceptron is that all errors are considered equally bad.
...
In particular, in Figure~\ref{fig:srl:trellis}, all of the edges that are not part of the gold standard path---those that are thinner and grey---get a free ``$+1$'' added to their weights.
...
\end{align}
Finally, we need to construct constraints.
...
The max denotes the fact that we are seeking the \emph{best} possible prefix up to position $l-1$, and forcing the label for position $l$ to be $k$.
